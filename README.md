...
可选口 `lora_trigger_words`：自动拼到每组提示词最前面（例如 `mh3turbo, <原提示词>`）。可接常见 LoRA 节点的触发词输出。卡片预览列可在 **采样预览** / **提示词预览** 之间切换，后者显示实际送进采样的全文（含公共提示词与触发词）。一采缓存指纹包含触发词；旧缓存若把触发词写进 prompt 仍可命中。
