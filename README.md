...
- 导演台 UI 中新建或缺少字段的时间轴默认开启段间引导（22 帧、默认「引导+重绘」、重绘幅度 0.65）、预览（速度 0.25）和全显模式；帧率固定为 24 fps
...
可选口 `lora_trigger_words`：可接常见 LoRA 节点的触发词输出。`r2v` 追加到文末独立块 `style_tags:`（避免模型把触发词当开场台词念出来）；其它模式仍拼到每组提示词最前面（例如 `mh3turbo, <原提示词>`）。卡片预览列可在 **采样预览** / **提示词预览** 之间切换，后者显示实际送进采样的全文（含该组提示词与触发词）。一采缓存指纹包含触发词；旧缓存若把触发词写进 prompt 仍可命中。
