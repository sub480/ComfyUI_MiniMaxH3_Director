...
新增 `task_type=mixed`。添加组后，每组可单独改类型（文生 `t2v` / 图生 `i2v` / 首尾帧 `fl2v` / 参考主体 `r2v` / 视频转视频 `v2v` / 参考素材改视频 `rv2v`），行为与对应原模式一致。`t2v` / `i2v` / `fl2v` 组走导演台主 `model`（fl2va）；`r2v` / `v2v` / `rv2v` 组请另接可选 `model_r2v`（ref2va），不接则回退到主模型。`v2v` / `rv2v` 组的源视频和专属素材均在组内设置。

`fl2v` 组可只填首帧、只填尾帧、两端都填或都不填（文生）。只填一端时**不会**把另一端补成同一张图。

### Director Prompt 同步

可选口 `director_prompt`：接收 `minimax-h3-director-prompt/v1` JSON（如 h3-director-export 输出），按组编排任务类型、时长、提示词、一采/二采和段间引导。工具栏「同步」读取上游 STRING 并刷新导演台分组；未连接时完全使用现有时间轴，连接后保留同序号组的已有素材槽位。每组时长须为 4–15 秒整数，并对齐 MiniMax `17k+5` 网格。

### 源视频编辑 (v2v / rv2v)

- 组内源视频时间轴可选定范围；上传、还原、分割和拖动范围后，帧数会**向下**对齐 MiniMax `17k+5` 网格（裁掉尾部多余帧，不补帧）；不足 5 帧的范围无效
- 采样与导出按该对齐后的源范围出片，不再带尾部多余帧
- 预览条支持 **Alt + 滚轮** 缩放，缩放比例与滚动位置会保留
- 播放器可拖动进度条跳帧、静音，并导出当前选中区间为 MP4

### 默认行为

- 视频组 / `fl2v` / 混合模式默认每组 5 秒，按固定 24 fps 对齐到 MiniMax `17k+5` 网格（通常为 124 帧）；可在组卡片中单独修改时长
- 一采默认 `seed=42`、8 步；输出画布默认 0.4MP、16:9（864×480）
- 导演台 UI 中新建或缺少字段的时间轴默认开启段间引导（22 帧、默认「引导+重绘」、重绘幅度 0.65、「保完整」关）、预览（速度 0.25）和全显模式；帧率固定为 24 fps
- 段间引导可勾选 **保完整**：仅对 `t2v` / `i2v` / `fl2v` / `r2v` 生效，保留对齐产生的尾部余帧，减少动作或台词结尾被裁断；开启后成片可能略长于时间轴。`v2v` / `rv2v` 始终按源区间导出
- 内置二采默认启用；`fl2v` 默认不跳过二采 / 放大，需要保护首尾帧时再勾选「跳过 fl2v」

### r2v 组参数与素材槽位

当前 r2v 已移除「公共参数」机制。每个素材组独立维护提示词、图片 1–9、视频 1–3 和音频 1–3；组内提示词中的 `<Picture N>` / `<Video K>` / `<Audio J>` 只引用本组素材，输入 `@` 也只打开本组素材菜单。外部 `Director Group (Reference to Video)` 接线同样按组提供素材，不再合并导演台的全局参考素材或提示词。

素材槽位采用统一的同类槽位管理：空槽点击上传/选择，已填槽点击预览，拖动可交换同类槽位；同一组不会重复添加同一路径的素材。参考视频显示首帧海报，参考音频可选择本地音频或视频（视频会提取音轨）。

### 导演台内置二采

导演台自带「二采」，不必再外接 Refine 节点。一采 / 二采 / 预览从节点原生控件收进输出栏，三个面板互斥（同时只开一个）：

- **一采配置**：种子、步数、采样器、调度器、`shift_video` / `shift_audio`
- **二采**：面板内勾选 **启用二采**（关 = 只一采，开后各组默认切到二采），以及模式 / 放大 / 分块等。`upscale` / `latent_upscale` 时按钮旁可直接改 **放大百万像素**
- **预览**：面板内勾选启用、调预览速度、选预览 VAE
- 底部运行状态显示一采 / 二采画布与分块（例如「一采 864×480 · 24fps」「二采 1280×720 · 放大 · h3_latent」）
...
- 可选口：`refine_model`（二采 UNET）、`refine_model_r2v`（仅 `r2v` / `v2v` / `rv2v`，连接时优先于 `refine_model`，未连接回退 `refine_model` 再回退该段一采模型）、`upscale_model`（仅 `upscale` + `lanczos`）、`refine_sigmas`（接线后覆盖步数 / 调度器 / denoise / 低噪加步）
- 仍可外接 **MiniMax H3 Director Refine**：接线后覆盖内置控件（旧工作流不用改）；Refine 节点同样可接 `refine_model_r2v`

### LoRA 触发词

可选口 `lora_trigger_words`：可接常见 LoRA 节点的触发词输出。`r2v` 追加到文末独立块 `style_tags:`（避免模型把触发词当开场台词念出来）；其它模式仍拼到每组提示词最前面（例如 `mh3turbo, <原提示词>`）。另有可选口 `lora_trigger_words_r2v`：仅供 `r2v` / `v2v` / `rv2v` 使用，连接时覆盖 `lora_trigger_words`，未连接则回退。卡片预览列可在 **采样预览** / **提示词预览** 之间切换，后者显示实际送进采样的全文（含该组提示词与触发词）。一采缓存指纹包含按任务类型解析后的触发词；旧缓存若把触发词写进 prompt 仍可命中。

### 分段一采 / 二采

启用二采后（内置面板或外接 Refine），每组卡片右上角可选 **一采** / **二采**（取代原先全局 `confirm_first_pass`）：

- **一采**：只跑第一遍；已有精确匹配的一采缓存则跳过
- **二采**：先一采再二采；已有精确匹配的一采缓存则直接二采
- 点状态色点可看该组是否匹配及差异项
- **清理**只删该组一采缓存
- 第 1 组从不钉上一段，其后加组或开关「段间引导」不会打掉组 1 的一采缓存
- 命中一采缓存时仍推送实时预览

分组卡片会展示全时间轴缓存状态。开启段间引导时，若没有上一段结果则跳过引导继续采样，而不会中断。
...
- 输出栏 **预览** 打开独立面板：启用开关、速度条（0–1，0 不显示，1 为原速 16fps）、预览 VAE
- 时间轴 / 分组卡片预览格式统一；卡片预览列可切 **采样预览** / **提示词预览**
- 视频缩略图懒加载首帧海报
- 二采逐步出预览；HD latent 先缩小再 TAE，避免二采预览比一采更吃显存
- 单显模式下预览列随卡片拉高

### 音频与帧率

- 声音输出（生成 / 原声 / 静音）按任务类型显示：`v2v` / `rv2v` 以及混合模式中的对应组可用原声
- 混合模式中 `v2v` / `rv2v` 组的原声取自该组本地源视频，而不是整条全局时间轴
- 原声长度跟随成片画面（含段间引导裁切后的长度）；开启「保完整」时生成段成片可能略长于时间轴，原声仍跟随成片
- 音频导出强制对齐 24 fps 网格
- 提示词文本框填充更稳
...
- 二采目标分辨率自动对齐（×32）
- 修复尾帧相关衔接问题；只填尾帧不再被当成首帧锁死
- 新增控件按名称写入工作流，避免插入二采分组后旧图控件错位
- 段间清显存常开（已去掉该开关）
- ffmpeg 编码改为一次写入并关闭 stdin，避免 Linux 上管道已关闭后再 flush 导致导出中断

### 缓存与状态检查

- 一采缓存指纹只记录提示词实际引用的图片 / 视频 / 音频槽位；未被 `<Picture N>`、`<Video K>` 或 `<Audio J>` 引用的槽位变化不会误使该组失效
- 开启段间引导时，指纹包含 `continuity_keep_tail`；`r2v` / `v2v` / `rv2v` 使用接线后的 `lora_trigger_words_r2v`（未接则回退 `lora_trigger_words`）
- 源视频按路径、文件大小和修改时间识别；源文件被替换后旧缓存不会继续作为成片使用
- 缓存状态查询只读取素材身份信息，不重新解码参考图或源视频，避免刷新卡片状态时抢占显存
- 参考图按文件身份复用解码结果；缓存读写失败只禁用缓存，不会中断正常采样
- 快照还原导致参考图片 / 音频 / 视频路径变化时，缓存可按媒体内容摘要识别相同文件；内容实际变化仍会使缓存失效
- 状态点区分缓存缺失 / 匹配 / 不匹配 / 元数据异常，点击或悬停可查看差异；状态扫描覆盖整个时间轴，不受「选择运行」限制
- 组内「清理」只清理该组一采缓存；输出栏「清理全部缓存」清理当前节点的一采、成片和音频缓存，操作前需确认并移入回收站
- 「全部导出」不会静默使用未运行片段的过期缓存；请将不匹配片段加入「选择运行」，或改用「分段导出」

### 导演台快照

- 工具栏「快照」按需打开管理窗口；可保存当前时间轴、素材和主要一采采样参数，并在列表中查看文件名、时间和大小
- 混合模式任务下拉可在当前 mixed 工作区与已保存快照之间切换；切到快照会先缓存当前工作区，切回 mixed 可还原。快照列表可独立滚动
- 快照保存在 `output/H3_D/snapshots/`，导出文件固定为 `*.mmxsnapshot.zip`
- 支持还原、重命名、复制和删除；还原会替换当前配置并要求确认，删除会将文件移入系统回收站
- 导入只要求文件名以 `.zip` 结尾，不要求使用 `.mmxsnapshot.zip` 后缀；内容仍须是有效的当前导演包
- 快照窗口内直接显示操作确认和错误提示，不会被快照界面遮挡

### 文件布局与数据格式

- `output/H3_D/snapshots/`：导演台快照
- `output/H3_D/segment_cache/<node_id>/`：分段一采、二采和音频缓存
- `output/H3_D/segment_export/<YYYYMMDD_HHMMSS>/`：按分段导出的 MP4
- `input/H3_D/uploads/`：导演台上传的源视频；`input/H3_D/references/`：参考图片 / 音频 / 视频；`input/H3_D/packs/<pack-id>/`：导入导演包素材
- UI 导出、快照和当前导演包采用规范化 camelCase 字段和 timeline v5；导演包导入要求 `pack.json`、`timeline.json` 及匹配的当前格式，不再根据目录文件名猜测缺失时间轴
- 上传、导入和缓存操作统一使用节点内提示框；快照操作在快照窗口内显示确认与错误

### 导演包与兼容性

- r2v 组素材在包内按组从 `Picture1` / `Video1` / `Audio1` 开始编号；不再因为所谓「公共参数」占用前 3 个图片槽而整体续编号
- 当前导演包使用 `pack.json` + `timeline.json` 保存完整时间轴；导入时严格校验当前包格式与 timeline v5
